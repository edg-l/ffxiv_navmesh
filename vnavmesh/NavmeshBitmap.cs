...
public sealed class NavmeshBitmap
{
    ...
    // a quad is an axis-aligned rectangle in the XZ plane at surface Y
    public void RasterizePolygon(GroundGraph.Quad quad)
    {
        if (quad.MinY > MaxBounds.Y || quad.MaxY < MinBounds.Y)
            return; // quad fully outside vertical bounds

        int x0 = Math.Clamp((int)MathF.Floor((quad.MinX - MinBounds.X) * Resolution), 0, Width - 1);
        int z0 = Math.Clamp((int)MathF.Floor((quad.MinZ - MinBounds.Z) * Resolution), 0, Height - 1);
        int x1 = Math.Clamp((int)MathF.Ceiling((quad.MaxX - MinBounds.X) * Resolution), 0, Width - 1);
        int z1 = Math.Clamp((int)MathF.Ceiling((quad.MaxZ - MinBounds.Z) * Resolution), 0, Height - 1);
        for (int z = z0; z <= z1; ++z)
            for (int x = x0; x <= x1; ++x)
                this[x, z] = false;
    }
